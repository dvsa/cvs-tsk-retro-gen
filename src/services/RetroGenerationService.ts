    ...
    for (let i = 0, j = 0; i < template.reportTemplate.activityDetails.length && j < activitiesList.length; i++, j++) {
      ...
      if (event.activityType === ActivityType.TEST) {
        ...
        if (defects && defects.length) {
            ...
            defectsDetails = defectsDetails + " " + defect.deficiencyRef + " (" +
            defect.deficiencyCategory + prsString +
            (defect.additionalInformation.notes ? (", " +
            defect.additionalInformation.notes) : "") + (defect.prohibitionIssued ? ", Prohibition was issued" : ", Prohibition was not issued") + ")";
          });
        ...
        if (customDefects && customDefects.length) {
          customDefects.forEach((customDefect) => {
            customDefectDetails = customDefectDetails + " " + customDefect.referenceNumber + " (" +
        ...
        detailsTemplate.failureAdvisoryItemsQAIComments.value = defectsString
          + customDefectsString
          + reasonForAbandoning
