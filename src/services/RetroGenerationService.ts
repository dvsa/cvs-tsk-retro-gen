import { IActivitiesList, IActivity, ITestResults, ITestType } from "../models";
...
class RetroGenerationService {
  ...
  public async generateRetroReport(activity: IActivity): Promise<any> {
    ...
    const template = await this.fetchRetroTemplate(totalActivitiesLen);
    ...
    for (let i = 0, j = 0; i < template.reportTemplate.activityDetails.length && j < activitiesList.length; i++, j++) {
      ...
      if (event.activityType === ActivityType.TEST) {
        ...
        const testType: ITestType = testResult.testTypes;
        ...
        let customDefectDetails: string = "Custom Defects: ";
        let prsString: string = "";

        const { defects, customDefects } = testType;
        let { reasonForAbandoning, additionalCommentsForAbandon } = testType;
        if (defects && defects.length) {
          ...
          defectsString = `Defects: ${defectsDetails};\r\n`;
        }

        if (customDefects && customDefects.length) {
          customDefects.forEach((customDefect) => {
            customDefectDetails = customDefectDetails + " " + customDefect.referenceNumber + " (" + customDefect.defectName + (customDefect.defectNotes ? ", " + customDefect.defectNotes : "") + ")";
          });
          customDefectsString = `Custom Defects: ${customDefectDetails}; \r\n`;
        }

        if (reasonForAbandoning) {
          reasonForAbandoning = `Reason for abandoning: ${reasonForAbandoning};\r\n`;
        }
        if (additionalCommentsForAbandon) {
          additionalCommentsForAbandon = `Additional comments for abandon: ${additionalCommentsForAbandon};\r\n`;
        }
    ...
    return {
      fileName:
        "RetrokeyReport_" + moment(activity.startTime).tz(TimeZone.LONDON).format("DD-MM-YYYY") + "_" + moment(activity.startTime).tz(TimeZone.LONDON).format("HHmm") + "_" + activity.testStationPNumber + "_" + activity.testerName + ".xlsx",
      fileBuffer: buffer,
    };
  }
  ...
  public computeActivitiesList(testResultsList: ITestResults[], waitActivitiesList: IActivity[]) {
    ...
    for (const testResult of testResultsList) {
      const act: IActivitiesList = {
        startTime: testResult.testTypes.testTypeStartTimestamp as string,
   ...
   */
  public async fetchRetroTemplate(totalActivities: number) {
    const workbook = new Excel.Workbook();
    const template = await workbook.xlsx.readFile(path.resolve(__dirname, "../resources/retro_report_template.xlsx"));
    // Index starts at 1
